...
db.define_table('auth_user',
                SQLField('nickname', label='Username', unique=True,
                         requires=[IS_NOT_EMPTY(), IS_NOT_IN_DB(db, 'auth_user.nickname', error_message='This name is already taken.'), 
                ...
                SQLField('email', length=128, default='', requires=[IS_EMAIL(), IS_NOT_IN_DB(db, 'auth_user.email'), IS_NOT_EMPTY()]),
...
db.define_table('oid_trust',
                SQLField('auth_user', db.auth_user),
...
def assignOidname():
    ...
    query = (db.auth_user.nickname == request.vars.nickname)
