...
def data():
    ...
    if 'allow_once' in request.vars:
        session.oid_trust_root_once = session.oid_orequest.trust_root
        return redirect(session.oid_url)
    if 'allow_always' in request.vars:
        query = (db.oid_trust.auth_user == auth.user.id) & (db.oid_trust.trust_root == session.oid_orequest.trust_root)
        assert db(query).count() == 0
        db.oid_trust.insert(auth_user = auth.user, trust_root = session.oid_orequest.trust_root)   
        return redirect(session.oid_url) 
